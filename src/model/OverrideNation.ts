...
export class OverrideNation implements INation {
    ...
    public static fabricateDummyNation(name: string, game: Game) {
        return new OverrideNation("REB", name + "?", "Rebel", game.getNationByTag("GER")!.getIdeas());
    }

    constructor(private tag: string, private name: string, private adjective: string, private ideas: NationalIdeaSet) {
    }

    makeImage(): HTMLImageElement {
        const url = Constants.getGfx("flags/" + this.tag + ".png");
